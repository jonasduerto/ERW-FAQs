(function() {
    tinymce.PluginManager.add('faqs_Shortcodes', function( editor, url ) {
        //editor.on('init', function(args){ERW_FAQ_Disable_Non_Premium();});
        editor.addButton( 'faqs_Shortcodes', {
            	...
            	onclick: function() {
				    var win = editor.windowManager.open( {
				        ...
				        },
				        {
				            ...
				            'values': ERW_FAQ_Create_Category_List('All')
				        },
				        {
            				type: 'listbox',
            				name: 'exclude_category',
            				label: 'Category to exclude:',
				            'values': ERW_FAQ_Create_Category_List('None')
				        }],
				        onsubmit: function( e ) {
				            ...
				            editor.insertContent( '[faqs '+post_text+' '+inc_cat_text+' '+excl_cat_text+']');
				        }
				    });
				}
			},
			{
            	text: 'Search FAQs',
            	onPostRender: function() {ERW_FAQ_Search_Non_Premium();},
            	value: 'ultimate-faq-search',
            	id: 'faq-search',
            	onclick: function() {
				    var premium = ERW_FAQ_Is_Premium();
				    if (!premium) {return;}
			...
			},
			{
            	...
            	onPostRender: function() {ERW_FAQ_Submit_Non_Premium();},
            	id: 'faq-question',
            	onclick: function() {
				    var premium = ERW_FAQ_Is_Premium();
				    if (!premium) {return;}
...
function ERW_FAQ_Create_Category_List(initial) {
	if (initial == "All") {var result = [{text: 'All', value: '-1'}];}
	else {var result = [{text: 'None', value: '-1'}];}
    var d = {};

	jQuery(faqs_categories).each(function(index, el) {
		var d = {};
		console.log(el);
		d['text'] = el.name;
		d['value'] = el.slug;
...
function ERW_FAQ_Search_Non_Premium() {
	var premium = ERW_FAQ_Is_Premium();
...
function ERW_FAQ_Submit_Non_Premium() {
	var premium = ERW_FAQ_Is_Premium();
...
function ERW_FAQ_Is_Premium() {
	if (faqs_premium == "Yes") {return true;}
